...
describe('ActualAiService', () => {
  ...
  describe('dry run mode', () => {
    ...
    beforeEach(() => {
      ...
      const batchTransactionProcessor = new BatchTransactionProcessor(transactionProcessor, 20, 2000);
    ...
    it('should NOT write #actual-ai-miss tag when LLM API error occurs in dry run mode', async () => {
      ...
      sut = new ActualAiService(
        ...
        dryRunApiService,
        new NotesMigrator(dryRunApiService, new TagService(NOT_GUESSED_TAG, GUESSED_TAG, MANUAL_OVERRIDE_TAG)),
      );
      await sut.classify();
